...
def process_file(model_id):
    ...
    for i in range(file.index.size):
        print(list(models[model_id-1]["model"].predict(np.array([file.iloc[i,:]]))[0]))
        res.append(list(file.iloc[i,:])+list(models[model_id-1]["model"].predict(np.array([file.iloc[i,:]]))))
